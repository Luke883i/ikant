...
def _contract_header(contract_text):
    ...
    return out
def _requires_presented_digest(contract_text):return _contract_header(contract_text).get('presented_terms_digest_handoff_required')=='true'
def validate_repository_admission_policy(root,contract_text=None):
    ...
    expected=policy_manifest();forbidden=set(expected['forbidden_before_acceptance']);caps=expected['orientation_capsule'];text=contract_text
    ...
    if h.get('schema')!=CONTRACT_SCHEMA:errs.append('contract schema mismatch')
    if version!=CONTRACT_VERSION:errs.append('contract version mismatch')
    if h.get('admission_policy_schema')!=expected['schema']:errs.append('contract admission policy schema mismatch')
    required_header={'pre_acceptance_default':'DENY','terms_envelope_path':TERMS_PATH,'orientation_capsule_enabled':'true','orientation_capsule_paths':'|'.join(caps['paths']),'orientation_max_file_reads':str(caps['max_file_reads']),'orientation_max_total_bytes':str(caps['max_total_bytes']),'orientation_max_metadata_reads':str(caps['max_metadata_reads']),'freeze_after_terms_presentation':'true','completed_access_accounting_required':'true','presented_terms_digest_handoff_required':'true','repository_materialization_requires_acceptance':'true','completed_forbidden_access_is_nonretroactive':'true','incidental_unexposed_overfetch_is_quarantined':'true','active_dashboard_egress_lock_required':'true','egress_delivery_ack_after_emit_required':'true','egress_pending_frame_replay_required':'true','egress_journal_hash_chain_required':'true','egress_guard_recreation_forbidden':'true','egress_breach_resume_transport_attestation_required':'true','active_machine_output_file_only':'true','egress_max_frame_bytes':str(MAX_FRAME_BYTES),'exit_command':EXIT_COMMAND,'resume_command':RESUME_COMMAND}
    ...
    e=a.get('active_human_egress',{});expected_egress={'schema':EGRESS_SCHEMA,'initial_state':'DASHBOARD_LOCKED','exclusive_human_output':True,'canonical_frame_only':True,'delivery_ack_after_emit_required':True,'pending_frame_durable':True,'journal_hash_chain':True,'guard_recreation_forbidden':True,'breach_resume_requires_transport_attestation':True,'machine_output_file_only':True,'max_frame_bytes':MAX_FRAME_BYTES,'exit_command':EXIT_COMMAND,'resume_command':RESUME_COMMAND,'breach_state':'EGRESS_BREACHED','legacy_v10_migration':'supported','resume_requires_runtime_integrity':True}
    ...
    if be.get('schema')!=EGRESS_SCHEMA or be.get('exclusive') is not True:errs.append('bootstrap egress policy mismatch')
    ...
    if be.get('guard_recreation_forbidden') is not True or be.get('breach_resume_requires_transport_attestation') is not True or be.get('machine_output_file_only') is not True:errs.append('bootstrap egress hardening mismatch')
    if be.get('max_frame_bytes')!=MAX_FRAME_BYTES:errs.append('bootstrap egress frame bound mismatch')
    if be.get('exit_command')!=EXIT_COMMAND or be.get('resume_command')!=RESUME_COMMAND or be.get('state_path')!='.ikant/egress.json':errs.append('bootstrap egress command/path mismatch')
...
def issue_receipt(contract_text,user_message,*,presented_terms_sha256=None,actor_type='human',evidence_type='explicit_user_message'):
    ...
    nonce=secrets.token_hex(16);accepted=now();esha=hashlib.sha256(user_message.encode()).hexdigest();schema='ikant-admission-receipt/v0.2' if presented_terms_sha256 else 'ikant-admission-receipt/v0.1';rid='ADM-'+hashlib.sha256(f'{csha}|{presented_terms_sha256 or ""}|{esha}|{accepted}|{nonce}'.encode()).hexdigest()[:16];out={'schema':schema,'receipt_id':rid,'contract_sha256':csha,'accepted_phrase':ACCEPT,'actor_type':actor_type,'evidence_type':evidence_type,'evidence_sha256':esha,'accepted_at':accepted,'nonce':nonce}
...
def probe(root,sdir,contract_text):
    root=Path(root);sdir=Path(sdir);checks={};checks['PYTHON']={'status':'AVAILABLE' if sys.version_info>=(3,11) else 'UNAVAILABLE','detail':sys.version.split()[0]};checks['CONTRACT']={'status':'AVAILABLE' if (root/TERMS_PATH).exists() and digest((root/TERMS_PATH).read_text())==digest(contract_text) else 'UNAVAILABLE'};receipt_ok,receipt_errors=validate_receipt(load_receipt(sdir),contract_text);checks['ACCEPTANCE_BINDING']={'status':'AVAILABLE' if receipt_ok else 'UNAVAILABLE','detail':'; '.join(receipt_errors)};policy_ok,policy_errors=validate_repository_admission_policy(root,contract_text);checks['ADMISSION_POLICY']={'status':'AVAILABLE' if policy_ok else 'UNAVAILABLE','detail':'; '.join(policy_errors)}
    try:sdir.mkdir(parents=True,exist_ok=True);p=sdir/'probe-scratch';p.write_text('ok');ok=p.read_text()=='ok';p.unlink();checks['LOCAL_PERSISTENCE']={'status':'AVAILABLE' if ok else 'UNAVAILABLE'}
    except OSError as e:checks['LOCAL_PERSISTENCE']={'status':'UNAVAILABLE','detail':str(e)}
    checks['PACKAGE_LAYOUT']={'status':'AVAILABLE' if (root/'ikant'/'runtime.py').exists() else 'UNAVAILABLE'};overall='READY' if all(x['status']=='AVAILABLE' for x in checks.values()) else 'BLOCKED';pid='PRB-'+secrets.token_hex(8);return {'schema':'ikant-probe/v0.2','probe_id':pid,'at':now(),'overall':overall,'checks':checks,'contract_sha256':digest(contract_text),'consumed':False}
