...
def kill(f):
 ...
 if f=='snapshot_schema':ms['schema']='bad'
  ...
  ms=build_snapshot(session_id='S',browser_id='B',page_id='P',navigation_epoch=0,url='https://example.test/',controls=[{'tag':'input','role':'textbox','name':'Email','disabled':True}]);
  try: build_web_action(ms,verb='fill',target_id=ms['controls'][0]['control_id'],value='x'); return True
  ...
  try: build_snapshot(session_id='S',browser_id='B',page_id='P',navigation_epoch=0,url=bad);return True
  except ValueError:return True
 elif f=='control_nul':
  try:build_snapshot(session_id='S',browser_id='B',page_id='P',navigation_epoch=0,url='https://example.test/',controls=[{'tag':'button','name':'x\x00y'}]);return False
  except ValueError:return True
 ...
 elif f=='cross_snapshot_replay' or f=='navigation_epoch_replay':ms=build_snapshot(session_id='S',browser_id='B',page_id='P',navigation_epoch=1,url='https://example.test/',controls=[{'tag':'input','role':'textbox','name':'Email'}])
 elif f=='origin_drift':ms=build_snapshot(session_id='S',browser_id='B',page_id='P',navigation_epoch=0,url='https://other.test/',controls=[{'tag':'input','role':'textbox','name':'Email'}])
 elif f=='port_drift':ms=build_snapshot(session_id='S',browser_id='B',page_id='P',navigation_epoch=0,url='https://example.test:444/',controls=[{'tag':'input','role':'textbox','name':'Email'}])
 elif f=='scheme_drift':ms=build_snapshot(session_id='S',browser_id='B',page_id='P',navigation_epoch=0,url='http://example.test/',controls=[{'tag':'input','role':'textbox','name':'Email'}])
 elif f=='oversize_fill':
  try:build_web_action(s,verb='fill',target_id=s['controls'][0]['control_id'],value='x'*65537);return True
  except ValueError:return True
 elif f=='wrong_control_type':
  try:build_web_action(s,verb='fill',target_id=s['controls'][1]['control_id'],value='x');return True
  except ValueError:return True
 elif f=='unknown_verb':
  try:build_web_action(s,verb='eval');return True
  except ValueError:return True
 elif f=='raw_selector':
  try:build_web_action(s,verb='click',selector='button');return True
  except TypeError:return True
 elif f=='raw_script':
  try:build_web_action(s,verb='click',script='alert(1)');return True
  except TypeError:return True
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--mutations',type=int,default=100000);ap.add_argument('--tail',type=int,default=10000);ap.add_argument('--seed',type=int,default=883);a=ap.parse_args();rng=random.Random(a.seed);counts={x:0 for x in FAMILIES};survivors=[]
