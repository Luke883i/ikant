...
class ProductConvergenceV29Tests(unittest.TestCase):
 ...
 def test_s11_and_s12_are_preserved_and_s13_is_public_test_current(self):
  c=json.loads((ROOT/'PRODUCT_CONTRACT.json').read_text());self.assertEqual(c['constitutional_convergence'],'S13');self.assertEqual(c['slices'][-3]['id'],'S11');self.assertEqual(c['slices'][-3]['schema'],'ikant-experience-projection/v1.3');self.assertEqual(c['slices'][-2]['id'],'S12');self.assertEqual(c['slices'][-2]['schema'],'ikant-foundation/v1-test');self.assertEqual(c['slices'][-1]['id'],'S13');self.assertEqual(c['slices'][-1]['schema'],'ikant-public-experience/v1-test');self.assertEqual(c['slices'][-1]['evidence']['public_release'],'v1.0-public-test')
 ...
 def test_pwa_keeps_lineage_and_bumps_for_public_v1(self):
  sw=(ROOT/'ikant/web/sw.js').read_text();self.assertIn('ikant-s10bis-bootstrap-v1',sw);self.assertIn('ecf1-3-runtime-v30',sw);self.assertIn('foundation-v1-s12',sw);self.assertIn('public-v1-s13',sw)
