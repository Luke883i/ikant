...
_S10_INVARIANTS = (
    Invariant("EPW-001","epistemic_workspace","Surface B inspection is read-only and available only to the current S8 writer after exact acknowledgement of the last sealed frame; any pending frame, writer drift, frame drift or session drift fails closed.","CRITICAL","tests.test_epistemic_workspace_v28"),
    Invariant("EPW-002","epistemic_workspace","Surface B reads are same-session and cycle-bound, generated-path only and bounded for snapshot size, history, objects and artifact download; DOCX access requires a same-session/cycle JSON companion.","CRITICAL","tests.test_epistemic_workspace_v28"),
