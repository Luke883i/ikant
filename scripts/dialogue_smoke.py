...
def run_case(name,prompt,response,setup=None,horizon=None,atoms=None,expected=()):
    with tempfile.TemporaryDirectory() as td:
        rt=runtime(Path(td));setup and setup(rt);out=compile_cognitive_turn(rt,prompt,horizon=horizon,atoms=atoms or [],export_docx=True);mode=out['central_oracle']['regulative_mode'];assert mode in expected,(name,mode,expected)
