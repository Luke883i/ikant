...
if slice_ids!=['S1','S2','S3','S4','S5','S6','S7','S8','S9','S10','S10bis']:fail('product slice coverage drift')
if product.get('constitutional_convergence')!='S10bis':fail('product convergence drift')
...
for path in ('ikant/bootstrap_observability.py','ikant/bootstrap_runtime.py','ikant/bootstrap_http.py'):
 if not (ROOT/path).is_file():fail('missing S10bis module '+path)
