...
class BootstrapObservabilityV29Tests(unittest.TestCase):
 ...
 def test_spawn_oserror_is_typed_and_key_removed(self):
  with tempfile.TemporaryDirectory() as td:
   root=Path(td);server=root/'llama-server';model=root/'m.gguf';server.write_text('x');server.chmod(0o700);model.write_text('m')
   sup=EngineSupervisor(root/'.ikant',popen_factory=lambda *a,**k:(_ for _ in ()).throw(OSError('spawn denied')),port_factory=lambda:31337);binding={'manifest_sha256':'a'*64,'engine':{'path':str(server)},'model':{'path':str(model),'id':'Qwen'}}
