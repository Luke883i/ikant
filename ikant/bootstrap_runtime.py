...
_STEPS=('WEB_APP','MANIFEST','ENGINE_COMPONENT','MODEL_COMPONENT','ENGINE_PROCESS','ENGINE_READINESS','PRODUCT_SERVICE')

class ObservableProductBootstrapCoordinator(ProductBootstrapCoordinator):
    """S10bis adapter: S5/S9 remain canonical; bootstrap observations become durable and visible."""
    def __init__(self,*args,**kwargs):
        super().__init__(*args,**kwargs);self.bootstrap_journal=BootstrapJournal(self.root);self._attempt_id=new_attempt_id();self._diagnostic_step='WEB_APP';self._diagnostic_degraded=None;self._error_detail=None;self._terminal_steps=set();self._started_steps=set()
    ...
    def start_async(self):
        with self._lock:
            ...
            self._diag('WEB_APP','PASS','LOCAL_WEB_AVAILABLE','iKant local web application','loopback product surface is available while runtime preparation continues');self._diag('MANIFEST','START','RUNTIME_MANIFEST_LOAD','MODEL_RUNTIME.json','loading and validating pinned managed-runtime manifest')
            t=threading.Thread(target=self._prepare,name='ikant-product-bootstrap',daemon=True);self._thread=t;t.start()
    ...
    def _on_progress(self,event:Any):
        src=event if isinstance(event,dict) else {};phase=str(src.get('phase') or 'PREPARING').upper();target=str(src.get('target') or 'component');size=int(src.get('bytes') or 0) if isinstance(src.get('bytes'),int) and not isinstance(src.get('bytes'),bool) else 0
        if phase=='PLAN':
            if 'MANIFEST' not in self._terminal_steps:self._diag('MANIFEST','PASS','RUNTIME_MANIFEST_VERIFIED','MODEL_RUNTIME.json','pinned managed-runtime manifest loaded and validated')
            self._start_if_missing('ENGINE_COMPONENT','ENGINE_COMPONENT_CHECK','llama.cpp','checking local engine cache or pinned package')
        elif phase in {'DOWNLOADING','VERIFIED'}:
            step=self._component_step(target)
            if step=='MODEL_COMPONENT' and 'ENGINE_COMPONENT' not in self._terminal_steps:self._diag('ENGINE_COMPONENT','PASS','ENGINE_COMPONENT_VERIFIED','llama.cpp','engine component satisfied before model acquisition')
            self._start_if_missing(step,'COMPONENT_ACQUISITION_START',target,'checking cache or acquiring pinned component')
            self._diag(step,'PASS' if phase=='VERIFIED' else 'PROGRESS','COMPONENT_VERIFIED' if phase=='VERIFIED' else 'COMPONENT_DOWNLOADING',target,'digest verified' if phase=='VERIFIED' else 'downloading pinned component',bytes_count=size,total_bytes=self._planned_bytes or None)
            if step=='MODEL_COMPONENT' and phase=='VERIFIED':self._start_if_missing('ENGINE_PROCESS','ENGINE_PROCESS_START','llama-server','starting verified local engine process');self._start_if_missing('ENGINE_READINESS','ENGINE_READINESS_WAIT',target,'waiting for local health probe')
        super()._on_progress(event)
    def _pass_missing_runtime_gates(self):
        rows=(('MANIFEST','RUNTIME_MANIFEST_VERIFIED','MODEL_RUNTIME.json','manifest validated by S5'),('ENGINE_COMPONENT','ENGINE_COMPONENT_VERIFIED','llama.cpp','engine component verified by S5'),('MODEL_COMPONENT','MODEL_COMPONENT_VERIFIED','local LLM','model component verified by S5'),('ENGINE_PROCESS','ENGINE_PROCESS_RUNNING','llama-server','local engine process started by S5'),('ENGINE_READINESS','ENGINE_READINESS_READY','local LLM','constrained readiness probe passed'))
        for step,code,target,detail in rows:
    ...
    def _failure_step(self,code):
        if code in {'ENGINE_READINESS_TIMEOUT','ENGINE_EXITED_EARLY'}:return 'ENGINE_READINESS'
