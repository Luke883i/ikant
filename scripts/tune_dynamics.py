...
def evaluate(p: DynamicsParameters) -> dict:
    ...
    # Echo/familiarity: recurrence may make material available but must saturate and never alter evidence.
    ...
    # Spaced retrieval should consolidate more than immediate repeated retrieval, without touching evidence.
    ...
    # Negative feedback: mismatch must become revisitable and less stable, never more evidential.
    ...
    # Homeostasis: overload must be reduced, not amplified, and remain near the declared target envelope.
    ...
    # Epistemic dominance: strong grounded evidence must outrank weak maximally modulated material.
    ...
    # Decay should preserve stable traces more than unstable traces.
...
def main():
    grids={
      'activation_decay':[.18,.22,.25,.28,.32],
      'recurrence_activation_gain':[.025,.035,.045],
      'retrieval_activation_gain':[.15,.19,.23],
      'homeostatic_target':[.30,.34,.38],
    }
    candidates=[]
    ...
    out={
      'source_fingerprint':source_fingerprint(),'schema':'ikant-dynamics-tuning/v0.2',
      'method':'deterministic engineering grid; not biological parameter estimation',
      'candidate_count':len(candidates),
      'hard_invariants_passed':current['hard_invariants_passed'],
      'fitness':current['fitness'],
      'current_rank':current_rank,
      'near_best':near_best,
      'parameters':asdict(DEFAULT_DYNAMICS),
      'metrics':current['metrics'],
      'best_candidate':{'fitness':best[1],'parameters':asdict(best[2]),'metrics':best[3]['metrics']},
    }
    print(json.dumps(out,indent=2,sort_keys=True))
    return 0 if near_best else 1
