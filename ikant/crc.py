...
def _native_rows(semantic_slice: dict) -> dict[Layer, list[dict]]:
    out = {r: [] for r in CONCENTRIC_ORDER}
    for row in semantic_slice.get("nodes", []):
        try: layer = Layer(row.get("layer"))
...
def evaluate_reticulum(semantic_slice: dict, *, horizon: EpistemicHorizon | None = None, previous_neurofunctional_state: dict | None = None) -> dict:
    ...
    native = _native_rows(semantic_slice)
    ...
    for idx, ring in enumerate(CONCENTRIC_ORDER):
        ...
        if idx == 0 and not frames:
            boundary = [r for r in semantic_slice.get("nodes", []) if r.get("source_mode") in {"user", "repository", "document", "live"} and r.get("kind") != NodeKind.PRINCIPLE.value]
    ...
    ioa_errors = []
    for row in all_rows:
        if row.get("kind") not in {k.value for k in NodeKind}: ioa_errors.append(f"unregistered kind:{row.get('kind')}")
        if row.get("source_mode") not in SUPPORTED_SOURCE_MODES: ioa_errors.append(f"unregistered source:{row.get('source_mode')}")
        if row.get("layer") not in {r.value for r in CONCENTRIC_ORDER}: ioa_errors.append(f"unregistered ring:{row.get('layer')}")
    ioa = not ioa_errors
    ec = not horizon_errors and horizon.required_answer_type in SUPPORTED_ANSWER_TYPES
    tc = len(TRANSMISSIONS) == len(CONCENTRIC_ORDER) - 1 and any(l.turing_like_formalism for l in LEVELS)
    horizon_exceeded = bool(horizon_errors) or RING_INDEX[Layer(horizon.max_ring)] < max((RING_INDEX.get(Layer(r.get("layer")), 0) for r in all_rows), default=0)
