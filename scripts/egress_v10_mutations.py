...
def guard(td):return DashboardEgressGuard(Path(td)/'egress.json',runtime_session_id='S')
...
with tempfile.TemporaryDirectory() as td:
 g=guard(td);r=g.seal_frame(FRAME,kind='TURN');k['prefix']=not g.acknowledge_visible(r,'x'+FRAME)
with tempfile.TemporaryDirectory() as td:
 g=guard(td);r=g.seal_frame(FRAME,kind='TURN');k['release_flag']=not g.acknowledge_visible(replace(r,release_after_frame=True),FRAME)
with tempfile.TemporaryDirectory() as td:
...
with tempfile.TemporaryDirectory() as td:
 p=Path(td)/'egress.json';p.write_text(json.dumps({'schema':LEGACY_EGRESS_SCHEMA,'runtime_session_id':'S','state':'FRAME_PENDING','epoch':1,'frame_seq':1,'last_frame_sha256':'a'*64,'last_cycle_id':'C','last_kind':'TURN','updated_at':'x','breach_reason':None}));g=guard(td);k['legacy_pending']=g.state==EgressState.BREACHED
with tempfile.TemporaryDirectory() as td:
...
with tempfile.TemporaryDirectory() as td:
 g=guard(td);k['exit_exact']=g.classify_user_text(' EXIT IKANT')=='INTENT' and g.classify_user_text('exit ikant')=='INTENT'
assert all(k.values()),k
print({'ok':True,'mutants_killed':sorted(k),'count':len(k)})
