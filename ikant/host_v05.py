...
def runtime_egress_lock(runtime):
 with _EGRESS_LOCK_CREATION:
  ...
  if lock is None:
   lock=threading.RLock();setattr(runtime,'_incarnate_egress_lock',lock)
  return lock
...
def emit_incarnate_surface_a(runtime,cycle_id:str,text:str,*,intention_node_id=None):
 with runtime_egress_lock(runtime):
  return _emit_conforming_surface_a(runtime,cycle_id,text,intention_node_id=intention_node_id)
