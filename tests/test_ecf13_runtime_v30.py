...
class ECF13RuntimeV30Tests(unittest.TestCase):
 ...
 def test_ecf13_contract_registers_all_thirty_invariants(self):
  c=json.loads((ROOT/'docs/ECF1_3_ENGINEERING_CONTRACT.json').read_text());ids=[x['id'] for x in c['invariants']];self.assertEqual(ids,[f'ECF13-{i:03d}' for i in range(1,31)]);product=json.loads((ROOT/'PRODUCT_CONTRACT.json').read_text());self.assertEqual(product['constitutional_convergence'],'S11');self.assertEqual(product['slices'][-1]['id'],'S11');self.assertEqual(product['slices'][-1]['invariants'],ids)
