...
def finalize_planning(runtime:Any,cycle:dict[str,Any],practical:dict[str,Any],*,central:dict[str,Any])->dict[str,Any]:
    before={nid:float(getattr(node,"evidence",0.0)) for nid,node in getattr(runtime,"nodes",{}).items()}
    graph=build_plan_graph(runtime,practical.get("action_ledger",{}));plans=[]
    ...
    ledger={"schema":PLAN_LEDGER_SCHEMA,"cycle_id":cycle.get("cycle_id"),"intent_sha256":(cycle.get("semantic_slice") or {}).get("intent_sha256"),"plan_count":len(plans),"plans":plans,"decision_lattice":lattice,"overall_status":overall,"epistemic_authority":0.0,"execution_authority":0.0,"execution_performed":False,"approval_reusable_across_steps_or_turns":False,"boundaries":{"action_governance_cannot_be_upgraded":True,"same_turn_approval_is_not_plan_execution_token":True,"symbolic_world_is_not_observed_world":True,"counterfactual_sensitivity_is_not_real_world_causality":True,"pareto_lattice_has_no_scalar_utility":True,"host_revalidates_each_material_step":True}}
    ...
    state=getattr(runtime,"runtime",{}).setdefault("planning",{});state["last"]={"schema":PLANNING_SCHEMA,"cycle_id":cycle.get("cycle_id"),"plan_ledger_sha256":ledger["sha256"],"overall_status":overall,"nondominated_plan_ids":lattice.get("nondominated_plan_ids",[]),"authority":0.0}
