...
class EpistemicWorkspaceV28Tests(unittest.TestCase):
 ...
 def test_static_ui_and_http_preserve_s9_semantic_surface(self):
  js=(ROOT/'ikant/web/epistemic.js').read_text(encoding='utf-8');css=(ROOT/'ikant/web/epistemic.css').read_text(encoding='utf-8');http=(ROOT/'ikant/epistemic_http.py').read_text(encoding='utf-8');app=(ROOT/'ikant/local_app.py').read_text(encoding='utf-8');index=(ROOT/'ikant/web/index.html').read_text(encoding='utf-8');sw=(ROOT/'ikant/web/sw.js').read_text(encoding='utf-8');bootstrap_path=ROOT/'ikant/bootstrap_http.py';bootstrap=bootstrap_path.read_text(encoding='utf-8') if bootstrap_path.is_file() else '';reactive_path=ROOT/'ikant/reactive_http.py';reactive=reactive_path.read_text(encoding='utf-8') if reactive_path.is_file() else ''
  direct='epistemic_http' in app;composed='bootstrap_http' in app and 'make_epistemic_handler' in bootstrap and '.epistemic_http' in bootstrap;reactive_composed='reactive_http' in app and 'make_bootstrap_handler' in reactive and '.bootstrap_http' in reactive and 'make_epistemic_handler' in bootstrap and '.epistemic_http' in bootstrap
  self.assertEqual(index.count('id="dashboard"'),1);self.assertNotIn('dashboard',js);self.assertIn('/api/v4/epistemic/index',http);self.assertIn('/api/v4/epistemic/artifact',http);self.assertIn('make_handler',http);self.assertNotIn('do_POST',http);self.assertIn('Graph',js);self.assertIn('List',js);self.assertIn("event.code==='Space'",js);self.assertIn('bindingHeaders',js);self.assertNotIn('https://',js+css);self.assertIn('prefers-reduced-motion:reduce',css);self.assertIn('EpistemicWorkspaceCoordinator',app);self.assertTrue(direct or composed or reactive_composed);self.assertIn('const CACHE=',sw);self.assertIn('keys.filter(k=>k!==CACHE)',sw)
