...
def frame(text: str, *, cycle: str = "CYC-screen", kind: str = "TURN") -> dict:
    return {
        "text": text,
        "receipt": {
            "runtime_session_id": "SES-hotfix6",
            "epoch": 1,
            "frame_seq": 7,
            "kind": kind,
            "cycle_id": cycle,
            "frame_sha256": hashlib.sha256(text.encode()).hexdigest(),
...
class StructuredPrimaryProjectionTests(unittest.TestCase):
    def test_real_screenshot_shape_no_longer_collapses_validated_reply_to_pending(self):
        dashboard = "\n".join([
            "+----------------------------------------------------------------------------------------------+",
            "| SUPERFICIE A      [VALIDATED] ciclo CYC-screen                                      |",
            "| [prompt-like text] > iKant: Ciao! Come posso aiutarti oggi?                          |",
            "| SUPERFICIE B      [BOUND] CRC_SNAPSHOT_CYC-screen.docx                              |",
            "+----------------------------------------------------------------------------------------------+",
        ])
        self.assertEqual(project_primary_text(dashboard, "TURN"), "iKant: Ciao! Come posso aiutarti oggi?")
        wrapped = wrap_prepared_frame(frame(dashboard))
        self.assertEqual(wrapped["primary_text"], "iKant: Ciao! Come posso aiutarti oggi?")
        self.assertNotEqual(wrapped["primary_text"], PENDING_PRIMARY_TEXT)

    def test_live_turn_prefers_structured_surface_over_ascii_dashboard(self):
        dashboard = "\n".join([
            "| SUPERFICIE A      [VALIDATED] ciclo CYC-screen |",
            "| [presentation deliberately unparsable] |",
        ...
        self.assertEqual(wrapped["primary_text"], "iKant: Ciao, questa risposta arriva dal dato strutturato.")
        self.assertEqual(wrapped["primary_projection_source"], "STRUCTURED_SURFACE_A")
        self.assertTrue(wrapped["render_contract"]["structured_primary_preferred"])
        self.assertTrue(wrapped["render_contract"]["dashboard_parsing_is_compatibility_only"])

    def test_explicit_primary_is_bounded_and_identity_prefixed(self):
        dashboard = "| SUPERFICIE A      [PENDING] ciclo CYC-screen |"
        with self.assertRaises(ValueError):
            wrap_prepared_frame(frame(dashboard), primary_text="risposta senza identita")
        with self.assertRaises(ValueError):
            wrap_prepared_frame(frame(dashboard), primary_text="iKant: x\x00y")
...
class FastInferenceBoundaryTests(unittest.TestCase):
    ...
    def test_compact_generation_preserves_language_identity_and_one_repair(self):
        responses = iter([
            {"choices": [{"message": {"content": "Hello, I can help with your question."}}], "usage": {"prompt_tokens": 100, "completion_tokens": 9}},
            {"choices": [{"message": {"content": "Sono iKant; il motore Qwen3.5-0.8B genera la risposta locale senza autorita propria."}}], "usage": {"prompt_tokens": 120, "completion_tokens": 18}},
        ])
        payloads = []
        ...
        self.assertIn('"language":"Italian"', system)
        self.assertIn('"identity_first":true', system)
        self.assertLess(len(system), 2500)
        self.assertLessEqual(payloads[0]["max_tokens"], 110)
        self.assertEqual(payloads[0]["tools"], [])
        self.assertEqual(broker.last_completion_metrics["attempts"], 2)
        self.assertEqual(broker.last_completion_metrics["epistemic_authority"], 0.0)
        self.assertEqual(broker.last_completion_metrics["execution_authority"], 0.0)

    def test_local_service_turn_has_no_redundant_health_round_trip_and_supplies_structured_primary(self):
        source = (ROOT / "ikant" / "local_service.py").read_text(encoding="utf-8")
        turn = source.split("    def turn(self,user_text):", 1)[1].split("    def notice", 1)[0]
        self.assertNotIn("self.model.health()", turn)
        self.assertIn("wrap_prepared_frame(prepared,primary_text='iKant: '+surface)", turn)
        self.assertIn("_structured_primary_from_chat", source)
        self.assertIn("model_metrics", turn)
...
class VoiceRoundTripBindingTests(unittest.TestCase):
    def test_voice_path_remains_local_candidate_then_same_explicit_turn(self):
        ...
        self.assertIn("rec.processLocally=true", js)
