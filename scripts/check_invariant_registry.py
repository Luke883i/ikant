...
historical_prefix=['S1','S2','S3','S4','S5','S6','S7','S8','S9','S10','S10bis','S11','S12','S13','S13bis']
if slice_ids[:len(historical_prefix)]!=historical_prefix:fail('product historical slice prefix drift')
if not slice_ids or product.get('constitutional_convergence')!=slice_ids[-1]:fail('product convergence drift')
if set(product['slices'][-1].get('invariants') or [])-set(critical_ids()):fail(f'{slice_ids[-1]} invariant registry drift')
...
historical_materialized=historical_prefix[:11]
for name in ('ADMISSION.json','BOOTSTRAP.json'):
 ...
 if pc.get('schema')!=product['schema'] or pc.get('product_version')!=PRODUCT_VERSION or materialized!=historical_materialized:fail(f'{name} historical product contract prefix drift')
...
for path in ('ikant/bootstrap_observability.py','ikant/bootstrap_runtime.py','ikant/bootstrap_http.py','ikant/experience_projection.py','ikant/future_supply.py','ikant/foundation.py','ikant/public_v1.py','ikant/enduser_identity.py','ikant/web/foundation.js','ikant/web/foundation.css','ikant/web/public-v1.js','ikant/web/public-v1.css','ikant/web/enduser.js','scripts/public_v1_pairing_recovery_falsify.py','scripts/enduser_identity_falsify.py','tests/test_public_v1_pairing_recovery.py','tests/test_enduser_self_model_s14.py'):
