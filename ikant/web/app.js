...
function freezeActive(){ $('intent').disabled=true;$('send-button').disabled=true;$('voice-button').disabled=true;$('exit-button').disabled=true; }
async function recoverActiveFrame(){
  setError('active-error','');freezeActive();
  try{const frame=await api('/api/v1/frame');await renderFrame(frame);}catch(_error){}
}
...
async function pair(code){
  ...
  state.token=payload.bearer_token;sessionStorage.setItem('ikantBearer',state.token);history.replaceState(null,'',location.pathname+location.search);show('pair-panel',false);show('pair-reset',true);await refresh();
}
...
function buildAck(frame){
  const r=frame.receipt||{};const visible=$('dashboard').textContent;return {schema:'ikant-web-human-ack/v0.20-test',runtime_session_id:r.runtime_session_id,epoch:r.epoch,frame_seq:r.frame_seq,frame_sha256:r.frame_sha256,visible_text:visible,visible_text_sha256:r.frame_sha256,epistemic_authority:0.0,execution_authority:0.0};
}

async function renderFrame(frame){
  setError('active-error','');
  if(frame.released){state.frame=null;$('dashboard').textContent='';freezeActive();show('released-panel',true);return;}
  if(frame.schema!=='ikant-web-human-frame/v0.20-test') throw new Error('Unexpected human frame schema');
  state.frame=frame;$('dashboard').textContent=frame.text;show('released-panel',false);await api('/api/v1/frame/ack',{method:'POST',body:buildAck(frame)});$('intent').disabled=false;$('send-button').disabled=false;$('voice-button').disabled=false;$('exit-button').disabled=false;
}

async function loadActive(){show('admission-panel',false);show('active-panel',true);show('pair-reset',true);const frame=await api('/api/v1/frame');await renderFrame(frame);}
async function refresh(){
  ...
  catch(error){if(String(error.message).includes('pairing')){state.token='';sessionStorage.removeItem('ikantBearer');show('pair-panel',true);show('admission-panel',false);show('active-panel',false);}else setError('pair-error',error.message);}
}
...
$('init-button').addEventListener('click',async()=>{try{const frame=await api('/api/v1/initialize',{method:'POST',body:{}});show('admission-panel',false);show('active-panel',true);await renderFrame(frame);}catch(error){$('preactive-status').textContent=error.message;}});

$('turn-form').addEventListener('submit',async event=>{event.preventDefault();setError('active-error','');const text=$('intent').value;if(!text.trim())return;freezeActive();try{const frame=await api('/api/v1/turn',{method:'POST',body:{text}});$('intent').value='';await renderFrame(frame);}catch(_error){await recoverActiveFrame();}});
$('exit-button').addEventListener('click',async()=>{setError('active-error','');freezeActive();try{const frame=await api('/api/v1/turn',{method:'POST',body:{text:'EXIT IKANT'}});await renderFrame(frame);}catch(_error){await recoverActiveFrame();}});
$('resume-button').addEventListener('click',async()=>{setError('active-error','');try{const frame=await api('/api/v1/resume',{method:'POST',body:{text:'RESUME IKANT'}});await renderFrame(frame);}catch(_error){await recoverActiveFrame();}});

async function startVoice(){
  if(!navigator.mediaDevices||!window.MediaRecorder) throw new Error('voice-unavailable');
  const stream=await navigator.mediaDevices.getUserMedia({audio:true});state.chunks=[];state.recorder=new MediaRecorder(stream);state.recorder.ondataavailable=e=>{if(e.data&&e.data.size)state.chunks.push(e.data);};state.recorder.onstop=async()=>{stream.getTracks().forEach(t=>t.stop());const blob=new Blob(state.chunks,{type:state.recorder.mimeType||'audio/webm'});try{const out=await api('/api/v1/voice/transcribe',{method:'POST',raw:blob,contentType:blob.type||'audio/webm'});if(out.schema==='ikant-web-human-frame/v0.20-test'){await renderFrame(out);return;}$('intent').value=out.text;$('intent').focus();}catch(_error){await recoverActiveFrame();}finally{state.recording=false;$('voice-button').textContent='Voce input';}};state.recorder.start();state.recording=true;$('voice-button').textContent='Stop voce';
}
$('voice-button').addEventListener('click',async()=>{setError('active-error','');try{if(state.recording){state.recorder.stop();}else await startVoice();}catch(_error){state.recording=false;$('voice-button').textContent='Voce input';await recoverActiveFrame();}});
