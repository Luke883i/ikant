...
from .experience_projection import timing_start,timing_mark,timing_public
...
def compile_cognitive_turn(runtime,intent:str,*,limit:int=12,horizon:EpistemicHorizon|None=None,atoms:list[dict]|None=None,export_docx:bool=False,docx_path=None,timing_origin:float|None=None):
    origin=float(timing_origin if timing_origin is not None else time.perf_counter()); timing=timing_start(origin); timing_mark(timing,'TURN_ACCEPTED',origin); timing_mark(timing,'COGNITIVE_START',time.perf_counter())
    ...
    prepared_core=prepare_epistemic_core(runtime,intent,limit=limit); cycle=runtime.concentric_cycle(intent,limit=limit); calibration=calibrate_cycle(runtime,cycle); cog=runtime.runtime.setdefault('cognitive',{}); previous_neuro=cog.get('neurofunctional_state',{}); timing_mark(timing,'SEMANTIC_SLICE_DONE',time.perf_counter())
    crc=evaluate_reticulum(cycle['semantic_slice'],horizon=horizon,previous_neurofunctional_state=previous_neuro); epistemic_core=finalize_epistemic_core(runtime,cycle,crc,horizon=horizon,previous_neurofunctional_state=previous_neuro,prepared=prepared_core,calibration=calibration); timing_mark(timing,'CRC_DONE',time.perf_counter())
    ...
    practical=finalize_practical_reason(runtime,cycle,temporal_core=temporal_core,central=central,mined=mined,atoms=atoms,intention_node_id=intention.id); timing_mark(timing,'GOVERNANCE_DONE',time.perf_counter())
