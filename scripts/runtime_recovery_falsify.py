...
def _coverage_signature(case_index:int,seed:int)->tuple[int,int,int]:
 # SIGNATURE_SPACE-1 is coprime to SIGNATURE_SPACE. This seed-bound modular
 # permutation visits every declared semantic signature exactly once per cycle.
 # Randomness is reserved for simultaneous-fault co-occurrence below, so the
 # Product Boundary can truthfully assert no novelty after the finite lattice
 # has been covered instead of relying on coupon-collector luck.
 idx=(case_index*(SIGNATURE_SPACE-1)+(seed%SIGNATURE_SPACE))%SIGNATURE_SPACE
 family=idx%FAMILIES
 return idx,family,family%len(DOMAINS)
...
def modeled(total:int,tail:int,seed:int)->dict[str,Any]:
 ...
 for i in range(total):
  a=splitmix64(seed+i);idx,f,d1=_coverage_signature(i,seed);seen[idx]=1;counts[f]+=1;faults=2+(a>>61)%3
