...
def make_reactive_handler(service,pairing,*,assets_dir:Path,allowed_hosts:frozenset[str],expected_port:int):
 ...
 class Handler(Base):
  def _composed_asset(self,name):
   ...
   if name=='app.js':files=(assets_dir/'app.js',assets_dir/'epistemic.js',assets_dir/'bootstrap.js',assets_dir/'reactive-hybrid.js')
  ...
  def do_GET(self):
   ...
   if path=='/api/v9/work/current':
    if not self._guard():return
    try:self._json(200,store_for_root(service.root).projection(active_session(service.root)))
