...
class DevelopmentContinuityS16bisTests(unittest.TestCase):
    ...
    def test_bundle_gate_represents_s21_over_merged_pr56_s20(self):
        ...
        self.assertEqual(out['status'],'PASS');self.assertEqual(out['candidate_slices'],['S21']);self.assertEqual(out['candidate_registration_state'],'DEVELOPMENT_CANDIDATE');self.assertEqual(out['baseline_product_contract_current_slice'],'S20');self.assertEqual(out['product_contract_current_slice'],'S20');self.assertFalse(out['registered_candidate_is_not_merged_main']);self.assertTrue(out['composite_candidate_support']);self.assertTrue(out['ready_to_develop_candidate']);self.assertFalse(out['candidate_complete']);self.assertFalse(out['ready_to_advance']);self.assertEqual(out['candidate_entry_blockers'],[]);self.assertEqual(out['candidate_open_objectives'],['FND-010']);self.assertIn('FND-003',out['future_open_risks'])
        ...
        for fid in ('FND-017','FND-018','FND-019','FND-024','FND-025','FND-026','FND-008','FND-009'):self.assertEqual(findings[fid]['status'],'CLOSED')
        ...
        by={x['id']:x for x in bundle['roadmap']};self.assertEqual(by['S19']['state'],'MERGED_PR56');self.assertEqual(by['S20']['state'],'MERGED_PR56');self.assertEqual(by['S21']['state'],'IN_DEVELOPMENT_PR57')
