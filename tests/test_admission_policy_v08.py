...
class AdmissionPolicyV08(unittest.TestCase):
 ...
 def test_probe_blocks_version_drift(self):
  with tempfile.TemporaryDirectory() as td:
   root=self.fixture(td);contract=(root/'IKANT_ACCESS_CONTRACT.md').read_text();s=root/'.ikant';save_receipt(s,issue_receipt(contract,'I ACCEPT'))
   a=json.loads((root/'ADMISSION.json').read_text());a['contract_version']='0.7.0';(root/'ADMISSION.json').write_text(json.dumps(a))
