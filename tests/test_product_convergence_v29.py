...
class ProductConvergenceV29Tests(unittest.TestCase):
 ...
 def test_s11_is_preserved_and_s12_is_current_foundation(self):
  c=json.loads((ROOT/'PRODUCT_CONTRACT.json').read_text());self.assertEqual(c['constitutional_convergence'],'S12');self.assertEqual(c['slices'][-2]['id'],'S11');self.assertEqual(c['slices'][-2]['schema'],'ikant-experience-projection/v1.3');self.assertEqual(c['slices'][-1]['id'],'S12');self.assertEqual(c['slices'][-1]['schema'],'ikant-foundation/v1-test')
 ...
 def test_pwa_keeps_s10bis_ecf13_lineage_and_bumps_for_foundation(self):
  sw=(ROOT/'ikant/web/sw.js').read_text();self.assertIn('ikant-s10bis-bootstrap-v1',sw);self.assertIn('ecf1-3-runtime-v30',sw);self.assertIn('foundation-v1-s12',sw)
