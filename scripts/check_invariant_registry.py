...
if product.get('schema')!='ikant-product-contract/v0.23-test' or product.get('product_version')!=PRODUCT_VERSION:fail('product contract drift')
if [x.get('id') for x in product.get('slices',[])]!=['S1','S2','S3','S4','S5']:fail('product slice coverage drift')
model_runtime=load_manifest(ROOT/'MODEL_RUNTIME.json')
if model_runtime.get('schema')!=MODEL_RUNTIME_SCHEMA or model_runtime.get('product_version')!=PRODUCT_VERSION:fail('managed runtime manifest drift')
for name in ('ADMISSION.json','BOOTSTRAP.json'):
 ...
 if pc.get('schema')!=product['schema'] or pc.get('product_version')!=PRODUCT_VERSION or pc.get('materialized_slices')!=['S1','S2','S3','S4','S5']:fail(f'{name} product contract drift')
