...
class ProductConvergenceV27Tests(unittest.TestCase):
 ...
 def test_s9_workspace_primitives_survive_s10(self):
  html=(ROOT/'ikant/web/index.html').read_text(encoding='utf-8');js=(ROOT/'ikant/web/app.js').read_text(encoding='utf-8');css=(ROOT/'ikant/web/styles.css').read_text(encoding='utf-8');self.assertEqual(html.count('id="dashboard"'),1);self.assertIn('command-palette',html);self.assertIn('inspector',html);self.assertIn('setup-panel',html);self.assertIn('orbit-rail',html);self.assertIn('/api/v3/product/status',js);self.assertIn('localService===true',js);self.assertIn('processLocally:true',js);self.assertIn('prefers-reduced-motion',css);self.assertTrue((ROOT/'docs'/'S9_PRODUCT_EXPERIENCE.md').is_file())
