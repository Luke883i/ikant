...
def main():
 ...
 with tempfile.TemporaryDirectory() as td:
  ...
  integrity=rt.integrity();chat=controller.log.verify();assert integrity['ok'] and chat['ok'];state=Path(rt.state_dir);assert (state/'psyche.json').exists() and (state/'dashboard.json').exists();rt.close();from ikant.runtime import Runtime;reopened=Runtime(Path(td)/'repo'/'.ikant');assert reopened.integrity()['ok'];reopened.close()
