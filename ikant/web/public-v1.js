...
function setHidden(el,on){const next=Boolean(on);if(el&&el.hidden!==next)el.hidden=next;}
...
function syncAdmission(){keepAcceptanceWritable();const accepted=$('step-accept')?.classList.contains('done')===true,probed=$('step-probe')?.classList.contains('done')===true;const form=$('accept-form'),acceptButton=form?.querySelector('button[type="submit"]'),probe=$('probe-button'),init=$('init-button'),copy=$('admission-copy');if(acceptButton){setHidden(acceptButton,accepted);acceptButton.textContent='Accetta';}setHidden(probe,!accepted||probed);setHidden(init,!probed);if(form)form.classList.toggle('accepted',accepted);if(copy)copy.textContent=!accepted?'Scrivi I ACCEPT per continuare.':!probed?'Condizioni registrate. Ora verifica l’ambiente locale.':'Ambiente verificato. Puoi avviare iKant.';}
function installAdmissionRepair(){const steps=[$('step-accept'),$('step-probe')].filter(Boolean);const observer=new MutationObserver(syncAdmission);for(const step of steps)observer.observe(step,{attributes:true,attributeFilter:['class']});syncAdmission();}
