...
def verify_file(path: str | Path, expected_sha256: str) -> bool:
    p = Path(path)
    return p.is_file() and sha256_file(p) == str(expected_sha256)
...
def _safe_member_path(root: Path, name: str) -> Path:
    if not name or "\x00" in name:
        raise ComponentStoreError("archive member path invalid")
    pure = PurePosixPath(name)
    parts = tuple(part for part in pure.parts if part not in {"", "."})
    if pure.is_absolute() or not parts or any(part == ".." for part in parts):
        raise ComponentStoreError("archive traversal rejected")
    out = root.joinpath(*parts).resolve()
    try:
        out.relative_to(root.resolve())
    except ValueError as exc:
        raise ComponentStoreError("archive escaped extraction root") from exc
    return out
...
def safe_extract_tar(archive: str | Path, destination: str | Path) -> Path:
    ...
    if dest.exists():
        raise ComponentStoreError("engine destination already exists")
    tmp = Path(tempfile.mkdtemp(prefix=dest.name + ".extract-", dir=dest.parent))
    try:
        with tarfile.open(src, "r:gz") as tf:
            members = tf.getmembers()
            if not members:
                raise ComponentStoreError("engine archive empty")
            for member in members:
                target = _safe_member_path(tmp, member.name)
                if member.issym() or member.islnk() or member.isdev() or member.isfifo():
                    raise ComponentStoreError("archive links/devices are forbidden")
                if member.isdir():
                    target.mkdir(parents=True, exist_ok=True)
                    continue
                if not member.isfile():
                    raise ComponentStoreError("unsupported archive member")
                target.parent.mkdir(parents=True, exist_ok=True)
                source = tf.extractfile(member)
                if source is None:
                    raise ComponentStoreError("archive member unreadable")
...
def find_unique_regular(root: str | Path, basename: str) -> Path:
    ...
    if len(candidates) != 1:
        raise ComponentStoreError(f"expected exactly one regular {basename}, found {len(candidates)}")
    return candidates[0]
