...
from ikant.session_host import DashboardOnlySession
...
class CausalLedgerS18Tests(unittest.TestCase):
    ...
    def test_canonical_exact_ack_is_only_commit_boundary(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            prepared=session.finalize(cycle,GOOD,intention_node_id=out['intention_node_id']);self.assertEqual(causal_projection(rt)['active']['stage'],'FRAME_SEALED')
