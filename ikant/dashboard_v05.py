...
DASHBOARD_SCHEMA='ikant-humanistic-dashboard/v0.9-test'
...
def _with_egress(runtime:Any,dashboard:dict[str,Any])->dict[str,Any]:
 guard=existing_runtime_egress(runtime)
 return guard.attach_projection(dashboard) if guard else dashboard
...
def _fit(text:str,width:int)->str:return sanitize_shell_content(str(text))[:width].ljust(width)
...
def render_dashboard_ascii(dashboard:dict[str,Any],*,width:int=96)->str:
 ...
 if eg:
  out.append('| '+_fit(f"Sessione          {eg.get('state','?')} | output umano: SOLO DASHBOARD | epoch {eg.get('epoch','?')}",inner)+' |')
  out.append('| '+_fit(f"Comando uscita    {eg.get('exit_command','EXIT IKANT')} -> assistente locale | rientro: {eg.get('resume_command','RESUME IKANT')}",inner)+' |')
 ...
 out.append(line);a=inc.get('surface_a') or {};astatus=a.get('status','EMPTY');out.append('| '+_fit(f"SUPERFICIE A      [{astatus}] ciclo {inc.get('cycle_id') or '-'}",inner)+' |')
