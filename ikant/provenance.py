...
def bind_node_source(
    ...
    *,
    ...
    obs = ObservationRecord(obs_id, node_id, source_id, acquisition, hashlib.sha256(text.encode("utf-8")).hexdigest(), bool(independent))
    ...
    if obs_id not in claim["observation_ids"]: claim["observation_ids"].append(obs_id)
    if source_id not in claim["source_ids"]: claim["source_ids"].append(source_id)
...
def _bind_existing_nodes(runtime: Any) -> None:
    ...
    for node_id, node in runtime.nodes.items():
        ...
        if mode not in EXTERNAL_SOURCE_MODES | DERIVED_SOURCE_MODES: mode = "runtime_derived"
    ...
    for relation in getattr(runtime, "relations", {}).values():
        derivations.append({
            "id": str(getattr(relation, "id", "")), "source": str(getattr(relation, "source", "")), "target": str(getattr(relation, "target", "")),
            "kind": str(getattr(getattr(relation, "kind", None), "value", getattr(relation, "kind", ""))), "weight": float(getattr(relation, "weight", 0.0)), "creates_evidence": False,
        })
...
def provenance_quality(runtime: Any, node_id: str) -> float:
    state = _state(runtime); claim = state.get("claims", {}).get(node_id, {}); source_ids = list(claim.get("source_ids", []))
    if not source_ids: return 0.0
    rows = [state["sources"].get(s, {}) for s in source_ids]
    external = sum(bool(r.get("external")) for r in rows)
    independent = len({r.get("provenance_key") for r in rows if r.get("external")})
    return round(min(1.0, 0.55 * (external / len(rows)) + 0.45 * min(1.0, independent / 2.0)), 6)


def validate_provenance_graph(graph: dict[str, Any]) -> tuple[bool, list[str]]:
    errors: list[str] = []
    if graph.get("schema") != PROVENANCE_SCHEMA: errors.append("provenance schema")
    sources = graph.get("sources") or {}; observations = graph.get("observations") or {}; claims = graph.get("claims") or {}
    for sid, row in sources.items():
        if row.get("id") != sid: errors.append("source id mismatch")
        mode = row.get("source_mode")
        if mode in DERIVED_SOURCE_MODES and row.get("external") is True: errors.append("derived source promoted to external")
        if row.get("authority") != "ATTRIBUTION_ONLY": errors.append("source attribution promoted to epistemic authority")
    for oid, row in observations.items():
        if row.get("id") != oid or row.get("source_id") not in sources or row.get("node_id") not in claims: errors.append("observation binding")
        if row.get("creates_evidence") is not False: errors.append("provenance observation creates evidence")
    for node_id, row in claims.items():
        if any(x not in observations for x in row.get("observation_ids", [])): errors.append(f"claim observation missing:{node_id}")
        if any(x not in sources for x in row.get("source_ids", [])): errors.append(f"claim source missing:{node_id}")
    return not errors, list(dict.fromkeys(errors))


def materialize_provenance(runtime: Any) -> dict[str, Any]:
    _bind_existing_nodes(runtime); state = _state(runtime)
    graph = {
        "schema": PROVENANCE_SCHEMA, "runtime_session_id": runtime.runtime.get("session_id"),
        "sources": dict(sorted(state.get("sources", {}).items())), "observations": dict(sorted(state.get("observations", {}).items())),
        "claims": dict(sorted(state.get("claims", {}).items())), "derivations": list(state.get("derivations", [])),
        "boundary": {"provenance_is_attribution_not_external_evidence": True, "derived_sources_never_self_promote": True, "content_identity_and_source_identity_are_separate": True},
    }
    ok, errors = validate_provenance_graph(graph)
    if not ok: raise RuntimeError("provenance validation failed: " + "; ".join(errors))
    raw = json.dumps(graph, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8"); digest = hashlib.sha256(raw).hexdigest()
    external_sources = sum(bool(x.get("external")) for x in graph["sources"].values()); multi_source_claims = sum(len(x.get("source_ids", [])) > 1 for x in graph["claims"].values())
    summary = {"schema": "ikant-provenance-summary/v0.13-test", "sha256": digest, "source_count": len(graph["sources"]), "external_source_count": external_sources, "observation_count": len(graph["observations"]), "claim_count": len(graph["claims"]), "multi_source_claim_count": multi_source_claims, "epistemic_authority": 0.0}
    core = _runtime_core(runtime); core["provenance"] = summary
    if getattr(runtime, "durable", False):
        path = Path(runtime.state_dir) / "provenance.json"; atomic_json_write(path, graph); summary["path"] = str(path)
