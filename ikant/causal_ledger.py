...
_STAGE={'TURN_OPEN':0,'COGNITIVE_PREPARED':1,'USER_CHAT_BOUND':2,'SURFACE_A_VALIDATED':3,'VISIBLE_CHAT_BOUND':4,'FRAME_SEALED':5,'TURN_COMMITTED':6,'TURN_ABORTED':6}
_EXCLUDED_PREFIXES=('causal-ledger.jsonl','causal-undo/','egress.json','egress-events.jsonl','egress-frames/','chat/transcript.jsonl','artifacts/')
...
def _enabled(runtime:Any)->bool:return bool(getattr(runtime,'durable',False) and getattr(runtime,'state_dir',None))
...
def _sanitize_refs(refs:dict[str,Any]|None)->dict[str,Any]:
 out=dict(refs or {});stack=[out]
 while stack:
  cur=stack.pop()
  for k,v in list(cur.items()):
   if str(k).casefold() in _FORBIDDEN_KEYS:raise ValueError(f'private causal field forbidden: {k}')
   if isinstance(v,dict):stack.append(v)
   elif isinstance(v,str) and len(v.encode('utf-8'))>4096:raise ValueError('causal reference string exceeds bound')
 return out

def _rows(runtime:Any)->list[dict[str,Any]]:
 if not _enabled(runtime):return []
...
def verify(runtime:Any)->dict[str,Any]:
 if not _enabled(runtime):return {'schema':CAUSAL_LEDGER_SCHEMA,'ok':True,'enabled':False,'events':0,'turns':0,'active_turn_id':None,'last_sha256':'0'*64}
 rows=_rows(runtime);prev='0'*64;session=str(runtime.runtime.get('session_id') or '');turns={}
 for seq,row in enumerate(rows,1):
  ...
  if prior:
   ...
   pc,cc=prior.get('cycle_id'),row.get('cycle_id')
 ...
 return {'schema':CAUSAL_LEDGER_SCHEMA,'ok':True,'enabled':True,'events':len(rows),'turns':len(turns),'active_turn_id':active[0]['turn_id'] if active else None,'last_sha256':prev}

def _append(runtime:Any,event:str,turn_id:str,*,cycle_id:str|None=None,refs:dict[str,Any]|None=None):
 if not _enabled(runtime) or not turn_id:return None
...
def _capture_undo(runtime:Any,turn_id:str):
 ...
 manifest={'schema':UNDO_SCHEMA,'turn_id':turn_id,'runtime_session_id':str(runtime.runtime.get('session_id') or ''),'entries':entries,'total_bytes':total};manifest['sha256']=_sha(manifest);atomic_json_write(dest/'manifest.json',manifest);fsync_parent(dest/'manifest.json');return manifest
...
def _restore_undo(runtime:Any,turn_id:str):
 state=Path(runtime.state_dir);dest=_undo_root(runtime)/turn_id;manifest=json.loads((dest/'manifest.json').read_text(encoding='utf-8'));supplied=manifest.get('sha256');material=dict(manifest);material.pop('sha256',None)
...
def _legacy_surface_abort_if_needed(runtime:Any)->None:
 ...
 marker=runtime.runtime.get('egress_guard') if isinstance(runtime.runtime.get('egress_guard'),dict) else {};pending=(runtime.runtime.get('cognitive') or {}).get('pending_surface_a_cycle_id')
 if not marker.get('required') and not pending and current['event'] in {'SURFACE_A_VALIDATED','VISIBLE_CHAT_BOUND'}:_append(runtime,'TURN_ABORTED',current['turn_id'],cycle_id=current.get('cycle_id'),refs={'reason_code':'LEGACY_NON_ACK_SURFACE','committed':False})

def begin_turn(runtime:Any):
 if not _enabled(runtime):return None
 ...
 tid='CTR-'+secrets.token_hex(12);manifest=_capture_undo(runtime,tid);_append(runtime,'TURN_OPEN',tid,refs={'undo_manifest_sha256':manifest['sha256'],'undo_files':len(manifest['entries']),'undo_bytes':manifest['total_bytes']});return tid
...
def prepare_turn(runtime:Any,turn_id,out):
 if not _enabled(runtime) or not turn_id:return None
 ...
 pr=out.get('practical_reason') or {};refs={'intention_node_id':out.get('intention_node_id'),'cycle':_file_ref(Path(runtime.cycles_dir)/f'{cycle}.json'),'surface_b':_file_ref(out.get('surface_b_json')),'temporal_replay_sha256':((out.get('temporal_epistemics') or {}).get('replay') or {}).get('sha256'),'action_ledger_sha256':(pr.get('action_ledger') or {}).get('sha256'),'plan_ledger_sha256':((pr.get('planning') or {}).get('plan_ledger') or {}).get('sha256'),'execution_ledger_sha256':(((pr.get('execution_protocol') or {}).get('execution_ledger') or {}).get('sha256')),'cognitive_influence':influence,'cognitive_influence_sha256':_sha(influence)}
 row=_append(runtime,'COGNITIVE_PREPARED',turn_id,cycle_id=cycle,refs=refs);_remove_undo(runtime,turn_id);return row

def _current_for_cycle(runtime:Any,cycle_id:str):
 current=active_turn(runtime)
 if not current:return None
 if str(current.get('cycle_id') or '')!=str(cycle_id):raise RuntimeError('active causal cycle mismatch')
 return current

def bind_user_chat(runtime:Any,cycle_id:str,row):
 current=_current_for_cycle(runtime,cycle_id)
 return _append(runtime,'USER_CHAT_BOUND',current['turn_id'],cycle_id=cycle_id,refs={'chat_user_seq':row.get('seq'),'chat_user_sha256':row.get('sha256'),'intention_node_id':row.get('intention_node_id')}) if current else None

def bind_surface_a(runtime:Any,cycle_id:str,receipt):
 current=_current_for_cycle(runtime,cycle_id)
 return _append(runtime,'SURFACE_A_VALIDATED',current['turn_id'],cycle_id=cycle_id,refs={'response_id':receipt.get('response_id'),'surface_a_validated':True,'speech_act_not_evidence':True}) if current else None

def bind_visible_chat(runtime:Any,cycle_id:str,row):
 current=_current_for_cycle(runtime,cycle_id)
 return _append(runtime,'VISIBLE_CHAT_BOUND',current['turn_id'],cycle_id=cycle_id,refs={'chat_response_seq':row.get('seq'),'chat_response_sha256':row.get('sha256'),'response_id':row.get('response_id')}) if current else None

def bind_frame(runtime:Any,prepared):
 if not _enabled(runtime):return None
 receipt=dict(prepared.get('receipt') or {});cycle=str(receipt.get('cycle_id') or '')
 if not cycle:return None
 current=_current_for_cycle(runtime,cycle)
 if not current:return None
...
def _egress_ack(runtime:Any,cycle_id:str):
 path=Path(runtime.state_dir)/'egress-events.jsonl'
 if not path.is_file():return None
 rows=[json.loads(x) for x in path.read_text(encoding='utf-8').splitlines() if x.strip()];seals={}
 for row in rows:
  ...
  if row.get('event') in {'ACK_FRAME','ACK_RELEASE'}:
   ...
   if seal and str((seal.get('payload') or {}).get('cycle_id') or '')==cycle_id:return {'ack_event':row.get('event'),'ack_journal_sha256':row.get('sha256'),'frame_sha256':(row.get('payload') or {}).get('frame_sha256'),'sealed_kind':(seal.get('payload') or {}).get('kind'),'seal_journal_sha256':seal.get('sha256')}
 return None

def _surface_seen(runtime:Any,turn_id:str)->bool:return any(r['turn_id']==turn_id and r['event'] in {'SURFACE_A_VALIDATED','VISIBLE_CHAT_BOUND'} for r in _rows(runtime))

def finalize_exact_ack(runtime:Any,prepared):
 if not _enabled(runtime):return None
 receipt=dict(prepared.get('receipt') or {});cycle=str(receipt.get('cycle_id') or '')
 if not cycle:return None
 current=_current_for_cycle(runtime,cycle)
 if not current:return None
 if current['event']!='FRAME_SEALED':bind_frame(runtime,prepared);current=active_turn(runtime)
 ack=_egress_ack(runtime,cycle)
 if not ack:raise RuntimeError('causal terminal requires exact durable egress ACK')
 surface_seen=_surface_seen(runtime,current['turn_id']);recovered=str(receipt.get('kind') or '').upper()=='RECOVERY'
 if not surface_seen and not recovered:raise RuntimeError('causal commit requires validated Surface A before exact ACK terminal')
 event='TURN_COMMITTED' if surface_seen else 'TURN_ABORTED';reason='RECOVERED_SURFACE_ACK' if event=='TURN_COMMITTED' and recovered else ('RECOVERY_INTERRUPTION_ACK' if event=='TURN_ABORTED' else 'EXACT_VISIBLE_ACK')
...
def abort_turn(runtime:Any,*,reason_code:str):
 current=active_turn(runtime)
 if not current:return None
 restored=_restore_undo(runtime,current['turn_id']) if current['event']=='TURN_OPEN' else {'restored_files':0,'restored_bytes':0}
 if current['event']!='TURN_OPEN':_remove_undo(runtime,current['turn_id'])
...
def reconcile_restart(runtime:Any):
 if not _enabled(runtime):return {'state':'CLEAN','causal_enabled':False}
 current=active_turn(runtime)
 if not current:return {'state':'CLEAN','causal_enabled':True}
 if current['event']=='TURN_OPEN':row=abort_turn(runtime,reason_code='PREPARE_CRASH_ROLLBACK');return {'state':'ROLLED_BACK_PREPARE','turn_id':row['turn_id']}
 cycle=str(current.get('cycle_id') or '');ack=_egress_ack(runtime,cycle) if cycle else None
 if ack:
  seen=_surface_seen(runtime,current['turn_id']);event='TURN_COMMITTED' if seen else 'TURN_ABORTED';row=_append(runtime,event,current['turn_id'],cycle_id=cycle,refs={**ack,'terminal_reason':'POST_ACK_RESTART_RECONCILE','recovered_delivery':ack.get('sealed_kind')=='RECOVERY','committed':event=='TURN_COMMITTED'});return {'state':'TERMINAL_RECONCILED','event':event,'turn_id':row['turn_id'],'cycle_id':cycle}
 return {'state':'FORWARD_RECOVERY_REQUIRED','turn_id':current['turn_id'],'cycle_id':cycle,'stage':current['event']}

def causal_projection(runtime:Any):
