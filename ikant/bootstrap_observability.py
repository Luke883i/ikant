...
EVENT_SCHEMA='ikant-bootstrap-event/v0.29-test'
STATUS_SCHEMA='ikant-bootstrap-status/v0.29-test'
EVENTS_SCHEMA='ikant-bootstrap-events/v0.29-test'
MAX_EVENT_BYTES=16*1024
MAX_API_EVENTS=256
MAX_RAW_BYTES=8*1024*1024
MAX_ARCHIVES=4
STEPS=(
 ('WEB_APP','Web app locale'),('MANIFEST','Manifest runtime'),('ENGINE_COMPONENT','Engine locale'),
 ('MODEL_COMPONENT','Modello LLM locale'),('ENGINE_PROCESS','Processo LLM'),
 ('ENGINE_READINESS','Readiness LLM'),('PRODUCT_SERVICE','Servizio iKant'),
)
STEP_IDS=frozenset(x[0] for x in STEPS)
OUTCOMES=frozenset({'START','PROGRESS','PASS','FAIL','INFO'})
_REDACT=re.compile(r'(?i)(authorization\s*[:=]\s*bearer\s+|api[_-]?key\s*[:=]\s*|token\s*[:=]\s*|password\s*[:=]\s*)([^\s,;]+)')
...
def safe_text(v:Any,limit:int=1024)->str:
 ...
 s=_REDACT.sub(lambda m:m.group(1)+'[REDACTED]',s);s=_QUERY.sub(r'\1?[REDACTED]',s)
 return s[:limit]
def exception_chain(exc:BaseException,limit:int=6)->list[dict[str,str]]:
 ...
 while cur is not None and len(out)<limit and id(cur) not in seen:
  seen.add(id(cur));out.append({'type':type(cur).__name__,'message':safe_text(cur,768)});cur=cur.__cause__ or cur.__context__
 return out
...
def _canon(x:dict[str,Any])->bytes:return json.dumps(x,ensure_ascii=False,sort_keys=True,separators=(',',':')).encode()

class BootstrapJournal:
 def __init__(self,root:str|Path):
  self.root=Path(root).resolve();self.path=self.root/'.ikant'/'bootstrap-events.jsonl';self.path.parent.mkdir(parents=True,exist_ok=True)
  self._lock=threading.RLock();self._seq=0;self._tail='0'*64;self._integrity='OK';self._rotate();self._replay()
 @property
 def state(self)->JournalState:return JournalState(self._seq,self._tail,self._integrity)
 def _rotate(self):
 ...
 def append(self,*,attempt_id:str,attempt:int,step:str,outcome:str,code:str,target:Any='',detail:Any='',bytes_count:int|None=None,total_bytes:int|None=None,remediation=None,cause_chain=None):
  ...
  with self._lock:
   ...
   digest=hashlib.sha256(_canon(e)).hexdigest();e['event_sha256']=digest;raw=_canon(e)+b'\n'
 ...
 def _scan(self,attempt_id:str|None=None,after_seq:int=0,limit:int|None=None):
  out=[]
 ...
 def status(self,*,attempt_id:str,attempt:int):
  latest={}
  for e in self._scan(attempt_id):latest[str(e.get('step') or '')]=e
  rows=[];passed=failed=running=0
  for sid,label in STEPS:
   e=latest.get(sid);st=str(e.get('outcome')) if e else 'PENDING';passed+=st=='PASS';failed+=st=='FAIL';running+=st in {'START','PROGRESS','INFO'}
   rows.append({'id':sid,'label':label,'status':st,'code':e.get('code') if e else None,'target':e.get('target') if e else None,'detail':e.get('detail') if e else None,'bytes':e.get('bytes') if e else None,'total_bytes':e.get('total_bytes') if e else None,'remediation':e.get('remediation') if e else None,'seq':e.get('seq') if e else None})
  overall='BLOCKED' if failed else 'READY' if passed==len(STEPS) else 'PREPARING' if running or latest else 'STARTING';raw_ok=self.path.is_file() and self.path.stat().st_size<=MAX_RAW_BYTES
  return {'schema':STATUS_SCHEMA,'attempt_id':str(attempt_id),'attempt':max(1,int(attempt)),'overall':overall,'steps':rows,'summary':{'passed':passed,'failed':failed,'running':running,'total':len(STEPS)},'journal':{'path':'.ikant/bootstrap-events.jsonl','integrity':self._integrity,'last_seq':self._seq,'tail_sha256':self._tail,'raw_available':raw_ok},'silent_failure':bool(overall=='BLOCKED' and failed==0),'presentation_is_authority':False,'epistemic_authority':0.0,'execution_authority':0.0}
